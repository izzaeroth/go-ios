...
func (d *Channel) methodCallWithReply(ctx context.Context, selector string, auxiliary PrimitiveDictionary) (Message, error) {
	...
	if err != nil {
		golog.Info("failed starting invoking method", "channel_id", d.channelName, "error", err, "methodselector", selector)
		return msg, err
	}
	if msg.HasError() {
...
func (d *Channel) Dispatch(msg Message) {
	...
	if msg.PayloadHeader.MessageType == Methodinvocation {
		golog.Trace("dispatching", "selector", msg.Payload[0].(string))
		...
		if msg.IsFragment() {
			if defragmenter, ok := d.defragmenters[msg.Identifier]; ok {
				...
				if msg.IsLastFragment() {
					...
					if len(leftover) != 0 {
						golog.Error("Decoding fragmented message failed")
					}
					if err != nil {
						golog.Error("Decoding fragment")
					}
			...
			golog.Warn("Received message fragment without first message, dropping it")
			delete(d.responseWaiters, msg.Identifier)
			delete(d.defragmenters, msg.Identifier)
			return
		}
