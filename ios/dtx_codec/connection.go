...
func (dtxConn *Connection) Dispatch(msg Message) {
	...
	if msgDispatcher != nil {
		golog.Debug("msg dispatcher found", "dispatcher", fmt.Sprintf("%T", msgDispatcher))
		msgDispatcher.Dispatch(msg)
		return
	}

	golog.Error("no connection dispatcher registered for global channel", "msg", msg)
}
...
func (g GlobalDispatcher) Dispatch(msg Message) {
	...
	if msg.Payload != nil {
		...
		if "outputReceived:fromProcess:atTime:" == msg.Payload[0] {
			...
			if len(args) < 3 {
				golog.Warn("outputReceived:fromProcess:atTime: expected at least 3 arguments", "count", len(args))
				return
			}
			logBytes, ok := args[0].([]byte)
			if !ok {
				golog.Warn("outputReceived:fromProcess:atTime: expected []byte argument", "type", fmt.Sprintf("%T", args[0]))
				return
			}
			logmsg, err := nskeyedarchiver.Unarchive(logBytes)
			if err == nil {
				golog.Debug("outputReceived:fromProcess:atTime:", "msg", logmsg[0], "pid", args[1], "time", args[2])
			}
			return
		}
	}
	golog.Trace("Global Dispatcher Received", "payload", msg.Payload, "auxiliary", msg.Auxiliary)
	if msg.HasError() {
		golog.Error("global dispatcher received error", "error", msg.Payload[0])
	}
	...
	for {
		...
		if err != nil {
			...
			if err == io.EOF || strings.Contains(errText, "use of closed network") {
				golog.Debug("DTX Connection with EOF")
				return
			}
			golog.Error("error reading dtx connection", "error", err)
			return
		}
...
func SendAckIfNeeded(dtxConn *Connection, msg Message) {
	if msg.ExpectsReply {
		...
		if err != nil {
			golog.Error("Error sending ack", "error", err)
		}
	}
}
...
func (dtxConn *Connection) RequestChannelIdentifier(identifier string, messageDispatcher Dispatcher, opts ...ChannelOption) *Channel {
	...
	golog.Debug("Requesting channel", "channel_id", identifier)
	...
	golog.Debug("channel request reply", "reply", rply)
	if err != nil {
		golog.Error("failed requesting channel", "channel_id", identifier, "error", err)
	}
	golog.Debug("Channel open", "channel_id", identifier)
