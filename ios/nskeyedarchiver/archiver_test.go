...
func TestXCTestconfig(t *testing.T) {
	...
	if err != nil {
		golog.Error("error reading or processing fixture", "error", err)
		t.Fatal()
	}
	fmt.Print(result)
	golog.Info("generated uuid", "uuid", uuid.String())
	res, err := nskeyedarchiver.Unarchive([]byte(result))
	assert.NoError(t, err)
	golog.Info("unarchived object", "object", res)
	...
	assert.NoError(t, err)
	golog.Info("unarchived object", "object", unarchivedObject)
}
...
func TestNSNull(t *testing.T) {
	...
	golog.Info("unarchived object", "object", unarchivedObject[0])
