...
func Prepare(device ios.DeviceEntry, skip []string, certBytes []byte, orgname string, locale string, lang string) error {
	...
	golog.Info("device is activated", "activated", isActivated)
	...
	defer conn.Close()
	golog.Info("send flush request")
	re, err := check(conn.sendAndReceive(request("Flush")))
	if err != nil {
		return err
	}
	golog.Debug("flush response", "response", re)
	golog.Info("get cloud config")
	config, err := check(conn.sendAndReceive(request("GetCloudConfiguration")))
	if err != nil {
		return err
	}
	golog.Debug("get first cloudconfig", "config", config)
	hello, err := check(conn.sendAndReceive(request("HelloHostIdentifier")))
	if err != nil {
		return err
	}
	golog.Debug("hello response", "response", hello)
	...
	golog.Debug("set cloud config", "config", setCloudConfig)
	...
	golog.Debug("set response", "response", setResp)
	hello, err = check(conn.sendAndReceive(request("HelloHostIdentifier")))
	if err != nil {
		return err
	}
	...
	golog.Debug("cloud config config", "config", config)
	...
	if err != nil {
		...
		golog.Debug("escalate unsupervised error (expected)", "error", err)
	}
	hello, err = check(conn.sendAndReceive(request("HelloHostIdentifier")))
	if err != nil {
		return err
...
func setupSkipSetup(device ios.DeviceEntry) error {
	...
	if err != nil {
		golog.Debug("skip setup: nothing to remove")
	}
	err = afcConn.MkDir(skipSetupDirPath)
	if err != nil {
		golog.Warn("error creating dir")
	}
	...
	if golog.Enabled(slog.LevelDebug) {
		...
		golog.Debug("list of files", "files", f)
	}
	return nil
}
